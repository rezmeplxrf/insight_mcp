...
export const toolDefinitions: ToolDefinition[] = [
  ...
  {
    name: "get_newsfeed",
    description: "News feed. Retrieve latest financial news with optional keyword and field-specific filtering → Returns {last_update: number, total_items: number, current_items: number, page: number, has_next: boolean, data: [{title?: string, content?: string, link?: string, published_at: number, related_symbols?: string[]}]}.",
    method: "GET",
    pathTemplate: "/v3/newsfeed",
    schema: {
    keywords: z.string().describe("(Optional) Keywords to include items that match one of the keywords in the title, content, or related symbols. Values may be separated by spaces or commas. e.g. apple,tesla").optional(),
    source: z.string().describe("(Optional) Return only news items whose source exactly matches this value. Matching is case-insensitive.").optional(),
    related_symbols: z.string().describe("(Optional) Return news items that contain any matching related symbol. Values may be separated by spaces or commas. Exchange-qualified values such as NASDAQ:AAPL require the full symbol; symbol-only values such as AAPL match related symbols after ignoring the exchange prefix.").optional(),
    title: z.string().describe("(Optional) Return only news items whose title contains this value. Matching is case-insensitive and partial.").optional(),
    content: z.string().describe("(Optional) Return only news items whose content contains this value. Matching is case-insensitive and partial.").optional(),
    limit: z.number().int().describe("(Optional) Limit the number of news items returned. Must be an integer between 1 and 500. Default is 500.").optional(),
