...
export const toolDefinitions: ToolDefinition[] = [
  ...
  },
  {
    ...
    description: "Option quotes by strike or expiration. Retrieve option quote rows with bid/ask, Greeks, implied volatility, strike, expiration, and contract metadata. Provide at least one selector: strike, range, expiration, from, or to. When strike and range are both provided, strike takes precedence and range is ignored. Use the /v3/symbols/quotes endpoint with returned option codes when you need last trade price and volume (up to 10 codes per request). → Returns {underlying_code: string, last_update: number, last_price?: number, data: [{code?: string, type: string, strike_price: number, expiration: number, ask_price: number, bid_price: number, delta: number, gamma: number, theta: number, vega: number, rho: number, implied_volatility: number, theoretical_price: number, bid_iv: number, ask_iv: number}]}. Provide at least one selector: strike, range, expiration, from, or to. Use API params type, sortBy, and sort to narrow server-side, then filter for Greek thresholds. To get last trade price and volume of returned contracts, call get_quotes with the option codes (up to 10). For historical option price data use get_symbol_series.",
    method: "GET",
    pathTemplate: "/v3/options/quotes",
    schema: {
    code: z.string().describe("(Required) Symbol code in Exchange:Symbol format (e.g., NASDAQ:AAPL). Use search_symbols to find the correct code."),
    strike: z.number().min(0).describe("(Optional if another selector is provided) Strike price for exact match filtering.").optional(),
    expiration: z.string().describe("(Optional if another selector is provided) Exact expiration date. Ignored when from or to is provided. (Format: YYYY-MM-DD)").optional(),
    from: z.string().describe("(Optional) Minimum expiration date. When provided, expiration parameter is ignored.  (Format: YYYY-MM-DD)").optional(),
    to: z.string().describe("(Optional) Maximum expiration date. When provided, expiration parameter is ignored. (Format: YYYY-MM-DD)").optional(),
    range: z.number().int().min(1).describe("(Optional) Strike price range as a percentage of the current underlying price. For example, range=10 returns only options with strikes within ±10% of the current price. Ignored when strike is provided.").optional(),
    type: z.enum(["call", "put"]).describe("(Optional) Filter by option type.").optional(),
